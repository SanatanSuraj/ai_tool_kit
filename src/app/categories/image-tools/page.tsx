...
export default function ImageToolsPage() {
  ...
  // Mock data for image tools
  const imageTools = [
    {
      id: 'image-compressor',
      name: 'Image Compressor',
      description: 'Reduce image file size while maintaining quality for faster websites and smaller storage requirements.',
    ...
    },
    {
      id: 'image-resizer',
      name: 'Image Resizer',
      description: 'Resize and scale images to specific dimensions for various platforms and use cases.',
    ...
    },
    {
      id: 'image-cropper',
      name: 'Image Cropper',
      description: 'Crop and adjust images to create perfect thumbnails or profile pictures with precision.',
      icon: 'ScissorsIcon',
      color: 'green',
    ...
    },
    {
      ...
      description: 'Convert images between formats including JPG, PNG, WebP, and more for compatibility and optimization.',
      icon: 'ArrowPathIcon',
      color: 'purple',
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <section className="py-16 md:py-24 relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          ...
          <div className="absolute top-0 left-1/4 w-64 h-64 rounded-full bg-rose-100/30 blur-3xl"></div>
          <div className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full bg-orange-100/30 blur-3xl"></div>
          
          {/* Animated background elements */}
          <div className="absolute top-20 left-10 w-2 h-2 rounded-full bg-rose-400 opacity-40 animate-pulse"></div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto">
            <div className="flex justify-center mb-4 scale-in-center">
              <div className="bg-rose-100 p-3 rounded-xl shadow-sm shadow-rose-200">
                <PhotoIcon className="h-8 w-8 text-rose-600" />
              </div>
            </div>
            
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 fade-in-bottom">Image Tools</h1>
            
            <p className="text-xl text-gray-600 mb-8 fade-in-bottom [animation-delay:200ms]">
              Powerful utilities to edit, optimize, and transform your images online. Make your visuals stand out effortlessly.
            </p>
            
            <div className="flex justify-center fade-in-bottom [animation-delay:300ms]">
              <Link href="/#all-tools-categories" className="group text-rose-600 font-medium flex items-center hover:text-rose-700 transition-colors">
              ...
              {imageTools.filter(tool => tool.popular).map((tool, index) => (
                <Link key={tool.id} href={tool.url} className="group">
                  <div className="bg-white rounded-xl shadow-sm hover:shadow-md p-6 border border-gray-100 h-full transition-all duration-200 group-hover:-translate-y-1 transform-gpu">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2 group-hover:text-rose-600 transition-colors">{tool.name}</h3>
                    <p className="text-gray-600">{tool.description}</p>
                    
                    <div className="mt-4 text-rose-600 font-medium flex items-center opacity-80 group-hover:opacity-100 transition-opacity">
                      Try Now
      ...
      <section className="py-8 pb-24">
        <div className="container mx-auto px-4">
          ...
          {searchTerm === '' && (
            <h2 className="text-xl font-semibold text-gray-800 mb-6 flex items-center">
              <span className="bg-rose-100 p-1.5 rounded-md mr-2">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(searchTerm !== '' ? filteredTools : imageTools).map((tool, index) => (
                <Link 
                  key={tool.id} 
                  href={tool.url} 
