    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <section className="py-16 md:py-24 relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          ...
          <div className="absolute top-0 left-1/4 w-64 h-64 rounded-full bg-green-100/30 blur-3xl"></div>
          <div className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full bg-teal-100/30 blur-3xl"></div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto">
            <div className="flex justify-center mb-4 scale-in-center">
              <div className="bg-green-100 p-3 rounded-xl shadow-sm shadow-green-200">
                <ArrowsRightLeftIcon className="h-8 w-8 text-green-600" />
              </div>
            </div>
            
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 fade-in-bottom">Converter Tools</h1>
            
            <p className="text-xl text-gray-600 mb-8 fade-in-bottom [animation-delay:200ms]">
              Convert between different units and formats with precision and ease. Our converter tools make transformations simple.
            </p>
            
            <div className="flex justify-center fade-in-bottom [animation-delay:300ms]">
              <Link href="/#all-tools-categories" className="group text-green-600 font-medium flex items-center hover:text-green-700 transition-colors">
        ...
        <div className="container mx-auto px-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 max-w-md mx-auto">
            <div className="relative">
              ...
              <input
                type="text"
                className="block w-full pl-10 pr-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm text-gray-900"
                placeholder="Search converter tools..."
                value={searchTerm}
              ...
              {converterTools.filter(tool => tool.popular).map((tool) => (
                <Link key={tool.id} href={tool.url} className="group">
                  <div className="bg-white rounded-xl shadow-sm hover:shadow-md p-6 border border-gray-100 h-full transition-all duration-200 group-hover:-translate-y-1 transform-gpu">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2 group-hover:text-green-600 transition-colors">{tool.name}</h3>
                    <p className="text-gray-600">{tool.description}</p>
                    
                    <div className="mt-4 text-green-600 font-medium flex items-center opacity-80 group-hover:opacity-100 transition-opacity">
                      Try Now
      ...
      <section className="py-8 pb-24">
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(searchTerm !== '' ? filteredTools : converterTools).map((tool, index) => (
                <Link 
                  key={tool.id} 
                  href={tool.url} 
