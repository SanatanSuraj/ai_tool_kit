...
export default function CheckerToolsPage() {
  const [searchTerm, setSearchTerm] = useState('');
  const [filteredTools, setFilteredTools] = useState<Tool[]>([]);
  const [isLoading, setIsLoading] = useState(true);

  // Mock data for checker tools
  const checkerTools = [
    {
      id: 'dns-lookup',
      name: 'DNS Lookup',
      description: 'Check DNS records and propagation status for any domain name.',
      icon: 'GlobeAltIcon',
      color: 'blue',
    ...
    },
    {
      id: 'whois-lookup',
      name: 'Whois Lookup',
      description: 'Retrieve domain name ownership and registration information.',
    ...
    },
    {
      ...
      description: 'Discover where a website is hosted and get server information.',
      icon: 'ServerIcon',
      color: 'indigo',
    ...
    },
    {
      id: 'ip-lookup',
      name: 'IP Lookup',
      description: 'Get detailed information about any IP address location and provider.',
      icon: 'MapPinIcon',
      color: 'pink',
      url: '/tools/ip-lookup',
      popular: true,
    },
    {
      id: 'ssl-checker',
      name: 'SSL Checker',
      description: 'Verify SSL certificates, expiration dates, and security information.',
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      ...
      <section className="py-16 md:py-24 relative overflow-hidden">
        <div className="absolute inset-0 -z-10">
          <div className="absolute inset-0 bg-gradient-to-br from-fuchsia-50/50 to-purple-50/50"></div>
          <div className="absolute top-0 left-1/4 w-64 h-64 rounded-full bg-fuchsia-100/30 blur-3xl"></div>
          <div className="absolute bottom-0 right-1/4 w-64 h-64 rounded-full bg-purple-100/30 blur-3xl"></div>
          
          {/* Animated background elements */}
          <div className="absolute top-20 left-10 w-2 h-2 rounded-full bg-fuchsia-400 opacity-40 animate-pulse"></div>
        ...
        <div className="container mx-auto px-4">
          <div className="text-center max-w-3xl mx-auto">
            <div className="flex justify-center mb-4 scale-in-center">
              <div className="bg-fuchsia-100 p-3 rounded-xl shadow-sm shadow-fuchsia-200">
                <ClipboardIcon className="h-8 w-8 text-fuchsia-600" />
              </div>
            </div>
            
            <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-6 fade-in-bottom">Checker Tools</h1>
            
            <p className="text-xl text-gray-600 mb-8 fade-in-bottom [animation-delay:200ms]">
              Verify and validate various types of data with our comprehensive checker tools. Get accurate information instantly.
            </p>
            
            <div className="flex justify-center fade-in-bottom [animation-delay:300ms]">
              <Link href="/#all-tools-categories" className="group text-fuchsia-600 font-medium flex items-center hover:text-fuchsia-700 transition-colors">
        ...
        <div className="container mx-auto px-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 max-w-md mx-auto">
            <div className="relative">
              ...
              <input
                type="text"
                className="block w-full pl-10 pr-3 py-2 border border-gray-200 rounded-lg focus:ring-2 focus:ring-fuchsia-500 focus:border-fuchsia-500 text-sm text-gray-900"
                placeholder="Search checker tools..."
                value={searchTerm}
        ...
        <section className="py-4 mb-8">
          <div className="container mx-auto px-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {checkerTools.filter(tool => tool.popular).map((tool) => (
                <Link key={tool.id} href={tool.url} className="group">
                  <div className="bg-white rounded-xl shadow-sm hover:shadow-md p-6 border border-gray-100 h-full transition-all duration-200 group-hover:-translate-y-1 transform-gpu">
                    ...
                    <h3 className="text-xl font-semibold text-gray-900 mb-2 group-hover:text-fuchsia-600 transition-colors">{tool.name}</h3>
                    <p className="text-gray-600">{tool.description}</p>
                    
                    <div className="mt-4 text-fuchsia-600 font-medium flex items-center opacity-80 group-hover:opacity-100 transition-opacity">
                      Try Now
      ...
      <section className="py-8 pb-24">
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {(searchTerm !== '' ? filteredTools : checkerTools).map((tool, index) => (
                <Link 
                  key={tool.id} 
                  href={tool.url} 
          ...
          {searchTerm !== '' && filteredTools.length === 0 && (
            <div className="text-center py-12">
              ...
              <button
                ...
                className="px-4 py-2 bg-fuchsia-50 text-fuchsia-600 rounded-lg hover:bg-fuchsia-100 transition-colors"
              >
                Clear Search
              </button>
            </div>
